...
- `claude/ui-audit.md` — measured per-screen design-system drift as-of `be6f343` (the state each redesign phase starts from), cross-cutting findings, Tier A/B/C ranking, and a STATUS split of closed defects vs open roadmap. **Read before touching any screen** — most drift there is expected (its phase has not run), so check it before "fixing" anything. Records the **planning gap: the create-antenna wizard is in no phase** (candidate P7).
...
- **No confirmation before project delete (queued, not fixed).** `LoadProjectScreen.kt:91` calls `ProjectStorage.deleteProject` straight from the card tap — no dialog, no undo, and the file is unrecoverable. This is a larger destructive-confirm gap than the calibration one, and the app now confirms before clearing a calibration but not before deleting a project.
