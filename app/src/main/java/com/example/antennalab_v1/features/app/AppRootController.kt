...
IMPORTANT
This file avoids Compose APIs and Android Context. There is no
stored-calibration restore policy: calibration is live-only, owned by
UsbSessionManager, and loading a project does not touch it.
########################################################################
*/
...
object AppRootController {
    ...
    */
    ...
    ------------------------------------------------------------
    SECTION 1600
    NO STORED-CALIBRATION RESTORE POLICY
    ------------------------------------------------------------
    There is deliberately no decideCalibrationRestore here.

    Calibration is live-only. Loading a project does not clear, restore,
    or otherwise touch the live calibration — the instrument's state is
    the instrument's business. The old policy resolved four of its five
    branches by DELETING live calibration, including when the project
    simply had nothing stored, which is how mere navigation destroyed a
    valid bench calibration.
    ------------------------------------------------------------
    */
}
