                    ...
                    ProjectSection.OVERVIEW -> {
                        ...
                        HardwareSelectionCard(
                            ...
                            onSelect = {
                                ...
                                workingProject = updatedProject
                                calibrationSession = ProjectWorkspaceController.buildFreshCalibrationSession(
...
/*
Workspace logic (sweep-return merge, calibration-session building, workflow
guidance, and formatting) lives in the pure, testable ProjectWorkspaceController.
These thin wrappers keep the existing call sites in the Composables unchanged.
*/
private fun mergeProjectReturnFromSweep(
    localProject: ProjectData,
    storedProject: ProjectData
): ProjectData = ProjectWorkspaceController.mergeProjectReturnFromSweep(localProject, storedProject)

private fun buildWizardCalibrationSession(
    project: ProjectData
): CalibrationSession = ProjectWorkspaceController.buildWizardCalibrationSession(project)
...
/*
The formatting + workflow-guidance helpers below now delegate to the pure,
testable ProjectWorkspaceController. They stay as private wrappers so the many
call sites inside the Composables above are untouched.
*/
private fun formatBudget(profile: BuildCostProfile) =
    ProjectWorkspaceController.formatBudget(profile)

private fun formatParts(profile: AvailablePartsProfile) =
    ProjectWorkspaceController.formatParts(profile)

private fun formatHardware(profile: TestHardwareProfile) =
    ProjectWorkspaceController.formatHardware(profile)

private fun formatClassification(classification: AntennaClassification): String =
    ProjectWorkspaceController.formatClassification(classification)

private fun antennaClassificationChoices(): List<AntennaClassification> =
    ProjectWorkspaceController.antennaClassificationChoices()

private fun formatSweepHistoryMode(mode: ProjectSweepHistoryMode): String =
    ProjectWorkspaceController.formatSweepHistoryMode(mode)

private fun formatTimestamp(epochMs: Long): String =
    ProjectWorkspaceController.formatTimestamp(epochMs)

private fun formatMillimetres(valueMm: Double): String =
    ProjectWorkspaceController.formatMillimetres(valueMm)

private fun buildNextAction(project: ProjectData): String =
    ProjectWorkspaceController.buildNextAction(project)

private fun buildReadinessSummary(project: ProjectData): String =
    ProjectWorkspaceController.buildReadinessSummary(project)

private fun buildUserViewSummary(project: ProjectData): String =
    ProjectWorkspaceController.buildUserViewSummary(project)
